...
class NetworkReceiver(private val caller: DownloadCompleteListener, val timestamp : String, val songList : ArrayList<Song>, val numSong: String) : BroadcastReceiver(){
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if(networkInfo?.type==ConnectivityManager.TYPE_WIFI){
            Download(caller,songList).execute(Songs,timestamp,numSong)
